...
export default function GolfSite() {
  ...
  const fetchTeeTimes = async () => {
    try {
      ...
      const imageRef = ref(db, 'scorecards');
      onValue(imageRef, (snapshot) => {
        const imageData = snapshot.val() || {};
        const images = Object.values(imageData);
        setScorecardImages(images);
      });
  ...
  const handleScorecardUpload = async (event) => {
    const files = Array.from(event.target.files);
    for (const file of files) {
      const storageReference = storageRef(storage, `scorecards/${Date.now()}_${file.name}`);
      await uploadBytes(storageReference, file);
      const url = await getDownloadURL(storageReference);
      await push(ref(db, 'scorecards'), url);
    }
  };
        ...
        <div style={{ color: '#2c3e50' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
            {scorecardImages.map((url, index) => (
              <img key={index} src={url} alt={`scorecard-${index}`} style={{ width: '200px', borderRadius: '8px' }} />
            ))}
          </div>
        </div>
      );
